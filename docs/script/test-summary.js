(function(){
  function toggle(ev) {
    ...
    while(parent) {
      if (parent.tagName === 'TR' && parent.classList.contains('validate-interface')) break;
  ...
  var buttons = document.querySelectorAll('.validate-summary tr.validate-interface .toggle');
  for (var i = 0; i < buttons.length; i++) {
    buttons[i].addEventListener('click', toggle);
  }

  var topDescribes = document.querySelectorAll('.validate-summary tr[data-validate-depth="0"]');
