        ...
        value: function create(ref, path) {
            ...
            var _functs = _signatures.map(function (_sig) {
                ...
                if (_sig.hasOwnProperty('*')) {
                    _this13.create(_sig['*'], path);
                    delete _sig['*'];
                    if (Object.keys(_sig) > 0) {
                        return _this13.create(_sig, path);
                    }
                    return;
                }
    ...
    _createClass(SchemaHelpers, [{
        ...
        value: function ensureRequiredFields(obj) {
            ...
            for (var _ in _required) {
                var _key2 = _required[_];
                var _path = this._ref.path.length ? this._ref.path : "root element";
                if (0 > oKeys.indexOf(_key2)) {
                    if (_exists(this._ref.signature[_key2].default)) {
                        obj[_key2] = this._ref.signature[_key2].default;
                    } else {
                        return 'required property \'' + _key2 + '\' is missing for \'' + _path + '\'';
                    }
                }
            }
            return obj;
