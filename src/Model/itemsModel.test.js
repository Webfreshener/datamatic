...
describe("ItemsModel Class Suite", function () {
    ...
    describe("Simple ItemsModel Tests", () => {
        beforeEach(() => {
            this.rxvo = new Model({schemas: [stringsCollection]});
        });
        ...
        describe("LifeCycle: Instantiation", () => {
            it("should initialize a schema and a schema object", () => {
                ...
                expect(this.rxvo.model.$model instanceof BaseModel).toBe(true);
            });
    ...
    describe("Nested Elements Tests", () => {
        beforeEach(() => {
            this.rxvo = new Model({schemas: [objectCollection]});
        });
        ...
        describe("LifeCycle: Delete", () => {
            beforeEach(() => {
                this.rxvo = new Model({schemas: [stringsMinMaxCollection]});
            });
    ...
    describe("Default Values", () => {
        it("should apply defaults to items", () => {
            this.rxvo = new Model({schemas: [objectCollectionDefaults]}, {ajvOptions: {useDefaults: true}});
            this.rxvo.model = [{}];
    ...
    describe("Model Class methods ", () => {
        ...
        it("should not reset if it would invalidate model", () => {
            const _rxvo = new Model({schemas: [stringsMinMaxCollection]});
        ...
        it("should reset it's collection if allowed", () => {
            const _rxvo = new Model({schemas: [stringsCollection]});
            _rxvo.model = ["Item A", "Item B", "Item C"];
            _rxvo.model = ["Item A", "Item B", "Item C"];
            expect(_rxvo.model.length).toBe(3);
            _rxvo.model.$model.reset();
        ...
        it("should freeze it's model hierarchy", () => {
            const _rxvo = new Model({schemas: [objectCollection]});
            const _orig = [{
                name: "My Name",
                active: true,
            }];
